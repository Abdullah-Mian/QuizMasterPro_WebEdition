...
INSERT INTO
    Student_Course (StudentID, Course_id)
SELECT
    s.StudentID,
    c.Course_id
FROM
...
CREATE TABLE
    OperationLog (
        LogID INT IDENTITY PRIMARY KEY, -- Unique identifier for each log entry
        rollno INT NOT NULL, -- Student roll number (or similar identifier)
        operation NVARCHAR (50) NOT NULL, -- Type of operation (e.g., 'INSERT', 'UPDATE')
        performed_by NVARCHAR (100) NOT NULL, -- User who performed the operation
        operation_date DATETIME DEFAULT GETDATE (), -- Timestamp of the operation
